...
%   EXPORT_REPO_IP(OUTPUT_REPO) overrides the destination repository path.
    ...
    model_dir = fullfile(repo_root, 'matlab', 'model');
    src_dir = fullfile(repo_root, 'matlab', 'src');
    build_root = fullfile(repo_root, 'matlab', 'build', 'repo_ip_prj');
...
function ensure_safe_generated_path(target_dir)
    ...
    repo_norm = lower(char(string(fileparts(fileparts(fileparts(mfilename('fullpath')))))));

    allowed_roots = { ...
        fullfile(repo_norm, 'matlab', 'build'), ...
